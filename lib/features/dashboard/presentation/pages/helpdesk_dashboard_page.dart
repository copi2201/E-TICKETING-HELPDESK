...
class _HelpdeskDashboardPageState extends State<HelpdeskDashboardPage> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final name = prefs.getString('name') ?? 'Tim Helpdesk';

      final stats = await _dashboardService.getHelpdeskStats();
                      ...
                      children: [
                          ...
                          children: [
                            Expanded(
                              child: _QuickActionButton(
                                ...
                                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => TicketListPage(role: UserRole.helpdesk, currentUserName: _userName))),
                              ),
                            ),
                            ...
                            Expanded(
                              child: _QuickActionButton(
                                ...
                                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ProfilePage(role: UserRole.helpdesk))),
                              ),
                            ),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: _QuickActionButton(
                                icon: Icons.notifications_none_rounded, title: 'Notification',
                                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const NotificationPage(role: UserRole.helpdesk))),
                              ),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                        ...
                        const SizedBox(height: 28),
                        const Text('Ringkasan Hari Ini', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 16),
                        GridView.count(
                          crossAxisCount: 2,
                          shrinkWrap: true,
                          ...
                          children: [
                            DashboardCard(title: 'Open', value: '${_stats['open'] ?? 0}', icon: Icons.mark_email_unread_outlined, color: const Color(0xFFFFA726)),
                            DashboardCard(title: 'Progress', value: '${_stats['progress'] ?? 0}', icon: Icons.timelapse, color: const Color(0xFF4F46E5)),
                            DashboardCard(title: 'Resolved', value: '${_stats['resolved'] ?? 0}', icon: Icons.check_circle_outline, color: const Color(0xFF10B981)),
                            DashboardCard(title: 'Urgent', value: '${_stats['urgent'] ?? 0}', icon: Icons.priority_high, color: const Color(0xFFEF4444)),
                          ],
                        ),
                        const SizedBox(height: 28),
                        const Text('Aktivitas Terbaru', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
  ...
  Widget _buildHeroCard() {
    final pending = _stats['pendingAction'] ?? 0;
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('HELPDESK OVERVIEW', style: TextStyle(color: Colors.white70, letterSpacing: 1.2, fontSize: 12)),
          const SizedBox(height: 12),
          Text('$pending Tiket\nMenunggu Tindakan', style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w800, height: 1.2)),
          const SizedBox(height: 12),
          const Text('Kelola laporan pengguna dengan cepat dan efisien.', style: TextStyle(color: Colors.white70)),
        ],
      ),
    );
  }
